...
model = HADGA(num_features, num_edge_features, cfg.use_edge)
model.to(device)
...
writer = SummaryWriter(log_dir=GRAD_DIR)
    ...
    while not minibatchIterator.end():
        ...
        for name, param in model.named_parameters():
            if param.requires_grad and param.grad is not None:
                writer.add_histogram(name + '/grad', param.grad, global_step=batch)
                # print(f'Gradient histogram for {name}:')
                # print(torch.histc(param.grad, bins=10, min=param.grad.min(), max=param.grad.max()))
