...
class HADGA(nn.Module):
    ...
    def _build(self):
        ...
        self.fc_layer = nn.Linear(8, cfg.class_num)
    ...
    def forward(self, placeholders, training=False):
        ...
        logist = torch.softmax(self.logist, dim=1)
        self.classes = placeholders['classes']
        # print("num_classes:", logist.size(1))
        self.pred_y = F.one_hot(torch.argmax(logist, dim=1), num_classes=logist.size(1))     # 将预测值转换成one-hot编码
        # for v in self.trainable_variables:
        #     if ':' not in v.name:
        #         print('unreadvariable')
        #         pdb.set_trace()
    ...
    def _loss(self):
        ...
        # 计算损失
        logs = criterion(self.logist.squeeze(), self.classes.float())
        ...
        self.loss = cfg.a * self.class_loss + cfg.g * self.reg_loss
        return self.loss, self.class_loss, self.reg_loss

    def _result_score(self):
        remain_index = self.placeholders['remain_index']    # list
        batch_classes = self.placeholders['batch_classes']      # [N_t]tensor
        pred_y = np.zeros((batch_classes.shape[0], np.int32(cfg.class_num)))  # 初始化预测标签, [batchsize]array
        pred_y[:, 0] = 1
        ...
        accuracy = Accuracy()
        accuracy.update(pred_y, batch_classes)
        accuracy = accuracy.compute().item()    # 获取准确率并转换为 NumPy 数组
        # print('acc', accuracy)
        recall = Recall()
        recall.update(pred_y, batch_classes)
        recall = recall.compute().item()
        # print('recall', recall)
        precision = Precision()
        precision.update(pred_y, batch_classes)
        precision = precision.compute().item()
        f1_score = 2.0 * ((precision * recall)/(precision + recall + 1e-9))
        # print('f1_score', f1_score)
        self.result_scores = [accuracy, recall, precision, f1_score]
