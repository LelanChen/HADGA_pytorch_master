...
from models.HADGA_without_T.layers import *
import numpy as np
import torch
...
class HADGA_without_T(nn.Module):
    def __init__(self, node_dim, edge_dim, use_edge):
        super(HADGA_without_T, self).__init__()
    ...
    def _build(self):
        ...
        self.embed_layer_1 = nn.Conv1d(in_channels=self.neighbors_layer_config[-1] * 2, out_channels=self.temporal_layer_config[-1], kernel_size=1)
        # 3: 创建全连接层，实现二元分类
    ...
    def forward(self, placeholders, training=False):
        ...
        outputs = self.embed_layer_1(self.temporal_input.permute(0, 2, 1))  # [N, F, T]
        outputs = outputs.permute(0, 2, 1)  # [N, T, F]
